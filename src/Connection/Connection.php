 ...
 * Unified connection class handling all drivers (MySQL, PostgreSQL, SQLite).
 * Replaces the 4-class hierarchy (Abstract + 3 drivers) with a single class
 * that uses match() for driver-specific behavior.
 *
 * PHP 8.4 property hooks for computed state properties.
 *
 * @copyright 2026 MonkeysCloud Team
 * @license   MIT
 */
final class Connection implements ConnectionInterface
{
    ...
    public function connect(): void
    {
        ...
        try {
            ...
            throw ErrorClassifier::classify($e, $this->connectionConfig->driver);
        }
    }
    ...
    public function pdo(): PDO
    {
        ...
        /** @var PDO $this->pdo — guaranteed non-null after connect() */
        return $this->pdo;
    }
    ...
    public function execute(string $sql, array $params = []): int
    {
        ...
        try {
            ...
            $this->eventDispatcher?->onQueryExecuted(
                $this,
                $sql,
                $params,
    ...
    private function applyMySqlDefaults(PDO $pdo): void
    {
        $charset = $this->connectionConfig->dsn->charset ?? 'utf8mb4';
        $pdo->exec("SET NAMES '{$charset}'");
        $pdo->exec("SET sql_mode = 'STRICT_TRANS_TABLES,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION'");

        if ($this->connectionConfig->timezone !== 'UTC') {
            $pdo->exec("SET time_zone = '{$this->connectionConfig->timezone}'");
        }
    }
    ...
    {
        $pdo->exec("SET NAMES 'UTF8'");
        $pdo->exec("SET timezone = '{$this->connectionConfig->timezone}'");
    }

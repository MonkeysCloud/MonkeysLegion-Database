...
final class ConnectionManager implements ConnectionManagerInterface
{
     ...
     * (both write and replica slots, including resolved lazy wrappers).
     *
     * @param callable(Connection): void $callback
     */
    private function propagateToOpenConnections(callable $callback): void
    {
        $apply = static function (ConnectionInterface $conn) use ($callback): void {
            if ($conn instanceof Connection) {
                $callback($conn);
            } elseif ($conn instanceof LazyConnection) {
                $inner = $conn->innerConnection();
